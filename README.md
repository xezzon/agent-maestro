...
第三方插件以 WASM 组件实现，经 https 来源（正式发布）或 file 来源（指向本机 manifest.json，本地调试）安装。面向插件作者的指南——manifest 字段语义、https 发布流约定、本地调试回路、`config_dir` 的声明语法与安全规则、沙箱边界——见 [docs/plugin-authoring.md](docs/plugin-authoring.md)；类型化绑定与依赖写法见 [crates/maestro-plugin-sdk](crates/maestro-plugin-sdk)。
