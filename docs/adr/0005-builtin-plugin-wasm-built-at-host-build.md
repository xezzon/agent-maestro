...
修订（2026-09-11）：本 ADR 末句「Git 来源的第三方插件仓库不受此决定约束——其产物形态由插件作者自行决定，宿主仅在装载与安装时做实例化校验。」随 ADR 0006 废弃 Git 来源而失效——第三方插件（https 与本机路径两种来源）同样不受本决定约束，其产物形态由插件作者自行决定，宿主仅在安装与装载时做实例化校验。
