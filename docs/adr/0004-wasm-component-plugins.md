...
修订（2026-09-20）：内置插件在安装上与其余来源一视同仁——wasm 字节仍内嵌于二进制，但启动时装载管线会为配置中缺失的内置插件落位 `~/.maestro/plugins/<id>/` 并写入配置条目，不再「不物化到磁盘」；「应用启动只读磁盘」随之改为「按配置条目落位装载 + 补装缺失内置插件」，全程不联网、离线可用。内置插件依旧不可移除；支持「重新加载」——按内嵌字节重建落位副本，可用于恢复被改动或损坏的宿主副本，配置条目（`enabled` 等）保持不变。
