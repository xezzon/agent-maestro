...
修订（2026-09-11）：第三方来源与装载模型改由 ADR 0006 决定，本 ADR 中关于 Git 来源与双恢复路径的描述随之失效——Git 来源（git2 克隆）整体废弃，第三方来源为指向 manifest.json 的 https URL 与本机绝对路径；「更新」与「重新加载」合并为同一个操作（按配置中的来源重新获取 manifest 与 wasm，成功才替换落位目录、失败时旧版本保持可用），并取消「一次重载所有插件」；两种来源统一落位 `~/.maestro/plugins/<id>/`，仅 `manifest.json` 原文与 `plugin.wasm` 两个文件，落位时不重写 entry。配置 `plugins` 段的条目为 `source` + `enabled` + `id`——`id` 在安装成功时写入，是来源到落位目录的唯一映射。信任模型的表述以「第三方插件」为准，不再限定 Git 来源。
