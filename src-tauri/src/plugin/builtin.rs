...
///
/// 产物与 manifest 来自仓库根的内置插件目录 `plugins/pi/`，
/// 经重建脚本（`plugins/pi/build.sh`）产出；CI 重编译比对防漂移。
...
pub const PI_WASM: &[u8] = include_bytes!("../../../plugins/pi/plugin.wasm");
...
mod tests {
    ...
    fn embedded_pi_manifest_is_valid_and_matches_builtin_identity() {
        ...
        assert!(!PI_WASM.is_empty(), "内置 pi wasm 产物必须提交入库");
    }
}
